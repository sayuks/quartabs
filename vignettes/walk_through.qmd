...
The default is `TRUE` and sort data by tabsest_vars.

```{r}
data.frame(
  tab = c("B", "A"),
  value = c("Tab content for B", "Tab content for A")
) |> 
  render_tabset(tab, value)
```


Whether to sort the data with tabset_vars. The default is `TRUE`. If `FALSE`, the tabset will be output in the original order of the data.
