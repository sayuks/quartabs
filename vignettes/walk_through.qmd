...
繰り返しを避けるために、デフォルトのチャンクオプションを変更しておくこともできる。
...
最初の引数は`tabset_vars`であり、tabに表示させる列名を指定する。2番目の引数は`output_vars`であり、tabの中に表示させる列を指定する。
...
例えばvar1は1と表示するが、list型のvar2は[1] 1と表示する。
:::
...
2025-03-05時点で、Quartoの最新バージョンは1.6だが、[Quarto 1.4で導入されたBootstrap 5.2.2を使用しているようだ](https://quarto.org/docs/download/changelog/1.4/#:~:text=for%20HTML%20output-,(%235210)%3A%20Update%20to%20Bootstrap%205.2.2,-(%235393)%3A%20Properly)。

bootstrap 5.2ではいくつかのtabのカスタマイズを利用できる。その1つが[pills](https://getbootstrap.com/docs/5.2/components/navs-tabs/#pills)である。
...
同様に[fill and justify](https://getbootstrap.com/docs/5.2/components/navs-tabs/#fill-and-justify)を3つから選ぶことができる。以下では、例のために長いラベルを作成し、半分の幅で表示する。
...
df1_long_label |>
  render_tabset(
    c(group1, group2),
    c(var1, var2, var3),
    tabset_width = "default" # "default" is the default.
  )
```
