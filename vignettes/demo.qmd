...
`render_tabset()` works in HTML format in .qmd files (not .Rmd files). So you need to be prepared to try the code.
...
`render_tabset()` is useful when you want to display similar results (e.g. figures and tables) for each group using tabsets.

The syntax for the `render_tabset()` function is as follows:
...
# `render_tabset()` converts factor to string and then executes cat() as follows.
cat(as.character(x))
```
...
<!-- Simply format the path to the saved figure like `![<caption>](<path/to/figure.png>)` and excute  `render_tabset()` as before. -->
