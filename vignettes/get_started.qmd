...
So, if these are included in `tabset_vars` or `output_vars`, it is converted internally to string (after sorting if `sort = TRUE`).
...
For example, we use `{plotly}` to create interactive figures. Simply apply `plotly::ggplotly()` to the already created ggplot object. **Then it is passed to `htmltools::div()`**.
